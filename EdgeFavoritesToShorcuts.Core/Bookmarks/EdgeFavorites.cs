...
namespace EdgeFavoritesToShortcuts.Core.Models
{
    public partial class EdgeFavorites
    {
        ...
        public static EdgeFavorites FromFile(string path)
        {
            try
            {
                EdgeFavorites favorites = JsonSerializer.Deserialize<EdgeFavorites>(File.ReadAllBytes(path), new JsonSerializerOptions()
                {
        ...
        private static IList<ExtractionError> ProcessUrl(string rootName, Favorite favorite, string destination)
        {
